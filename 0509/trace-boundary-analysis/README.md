...
- `scripts/run_experiments.py`：实验入口，负责规则层级对比、参数 sweep、ensemble sweep，并支持命令行或 config.yaml 加载参数。
...
`--error-analysis-output` 会在非 sweep 运行时输出 false positives、false negatives 和 true positives，适合在 sweep 后固定一组参数继续分析坏例。
...
当前基于 `ensemble_sweep_20260511_172039.md` 的实验结论是：`rule + unsupervised` 的 OR/AND 组合没有超过单独 `unsupervised` 的 precision/recall 前沿。默认 `unsupervised` 参数已调到高 precision 折中点：`--threshold 0.55 --bad-risk-threshold 0.45 --bad-risk-weight 0.60 --centrality-weight 0.20`。

sweep 会自动生成 Markdown 文件，默认输出到当前目录，文件名格式为：

```text
<方法名>_sweep_<YYYYMMDD_HHMMSS>.md
```
...
```powershell
python .\scripts\run_experiments.py <trace_json_or_dir> --metadata <metadata.csv> --sweep-hybrid --sweep-hybrid-thresholds 0.45,0.50,0.55 --sweep-hybrid-bad-risk-thresholds 0.50,0.55,0.60,0.65 --sweep-good-margins 0.00,0.05,0.10
```

常用扫描参数：

- `--sweep-repeat-thresholds`：rule 的重复/循环阈值。
- `--sweep-rule-layers`：rule 的知识层级，默认扫描 `general,trace_format,domain_prior`。
