...
```text
训练阶段：读取训练 trace -> 抽取通用/行为/schema/动态字段特征 -> 生成规则文件
测试阶段：读取测试 trace -> 抽取同一套特征 -> 加载规则文件 -> 输出预测和报告
```
...
| `non_llm_unlabeled` | 非 LLM | 无标注 trace | 从通用、行为、schema 和动态字段特征中生成异常阈值规则 | 通用规则 + `dynamic/non_llm/unlabeled_rules.json` |
| `non_llm_labeled` | 非 LLM | 有标注 trace | 比较 train 中 goodcase/badcase 的通用、行为、schema 和字段特征差异，生成区分规则 | 通用规则 + `dynamic/non_llm/labeled_rules.json` |
| `llm_no_train` | LLM | 无训练数据 | 让 LLM 根据通用特征说明生成保守通用候选规则 | 通用规则 + `dynamic/llm/no_train_rules.json` |
| `llm_unlabeled` | LLM | 无标注 trace | 让 LLM 阅读无标注训练样本的通用、行为、schema 和动态字段特征，生成异常型候选规则 | 通用规则 + `dynamic/llm/unlabeled_rules.json` |
| `llm_labeled` | LLM | 有标注 trace | 让 LLM 阅读带标签训练样本的通用、行为、schema 和动态字段特征，生成区分 good/bad 的候选规则 | 通用规则 + `dynamic/llm/labeled_rules.json` |
...
flowchart LR
    ...
    B -->|无标注训练数据| D["抽取训练特征<br/>通用 + 行为 + schema + 动态字段"]
    B -->|有标注训练数据| D
...
2. **行为特征**：由步骤序列和结果序列生成，例如 `behavior_action_switch_count`、`behavior_repeated_result_ratio`、`behavior_error_step_ratio`、`behavior_retry_action_count`。
3. **Schema profile 特征**：由 trace 结构生成，例如 `schema_leaf_field_count`、`schema_max_depth`、`schema_list_count`、`schema_empty_scalar_ratio`。
4. **动态字段特征**：由 `features.py` 自动遍历 trace 实际字段生成，例如：
   - `field_exists:status`
   - `field_text:status`
   - `field_nonempty_ratio:plan_list[].result`
   - `field_number_mean:metrics.score`
   - `field_bool_true_ratio:flags.completed`
   - `field_text_chars:plan_list[].result`
   - `field_unique_value_ratio:events[].tool`
   - `field_error_text_count:steps[].observation`
...
1. 对训练 trace 抽取通用特征、行为特征、schema profile 特征和动态字段特征。
2. 对通用风险特征、行为特征、schema profile 特征计算 90 分位阈值。
3. 对训练集中高频、非空的业务字段生成“字段缺失/字段为空”异常规则。
4. 对动态字段的细粒度统计生成实验性异常规则，例如字段值数量过高、文本过长、错误文本过多、值多样性过低。
5. 生成“相对当前训练样本异常”的 badcase 规则。
6. 如果训练样本中存在可采用的 final-answer 字段，则生成缺失 final answer 的风险规则。
7. 写入 `scripts/rules/dynamic/non_llm/unlabeled_rules.json`。
...
3. 分别计算两类样本的通用特征、行为特征和 schema profile 特征均值。
4. 比较动态字段特征：字段是否出现、字段是否缺失、短文本取值、数值大小、字段长度、值多样性、错误文本数量是否与 good/bad 明显相关。
5. 如果某个风险特征、结构特征、行为特征或字段信号在 badcase 中显著更高，则生成 badcase 阈值或字段规则。
